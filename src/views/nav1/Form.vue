<template>
	<section>
		...
		<el-table :data="villas" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column prop="name" label="型号名" width="120" sortable>
			</el-table-column>
			<el-table-column prop="modelID" label="ID" width="120" sortable>
			</el-table-column>
		...
		getModel: function(id) {
			...
			this.$http.post(host+"getElectronicsModelByBrand/", params, { headers: "Content-Type:application/json" }).then(function(response) {
				...
				for(var item in response.body){
					...
					var temp = response.body[item]
					tempmodel.modelID = temp.pk
					tempmodel.name = temp.fields._name
					tempmodel.brandID = temp.fields._brand
					tempmodel.deviceType = temp.fields._deviceType
					tempmodel.thumbnail = temp.fields._thumbnail
					this.villas.push(tempmodel)
				}
			}).catch(function(error) {
				console.log(error)
			})
